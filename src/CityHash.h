...
template <typename T> struct city_hash_t : public Hasher<city_hash_t<T>, T> {
...
template <typename T> bool city_hash_t<T>::has_sse4_2 = support_sse4_2();
...
template <>
const city_hash_32_t::hash_value_t city_hash_32_t::operator()(void *buf, size_t len,
                                                              city_hash_32_t::seed_value_t seed) const {
    return CityHash32WithSeed((const char *)buf, len, seed);
}

template <>
const city_hash_64_t::hash_value_t city_hash_64_t::operator()(void *buf, size_t len,
                                                              city_hash_64_t::seed_value_t seed) const {
    if (seed) {
...
template <>
const city_hash_128_t::hash_value_t city_hash_128_t::operator()(void *buf, size_t len,
                                                                city_hash_128_t::seed_value_t seed) const {
#if defined(__SSE4_2__) && defined(__x86_64__)
    if (has_sse4_2) {
        if (seed) {
            const uint128 &hash =
                CityHashCrc128WithSeed((const char *)buf, len, std::make_pair(U128_LO(seed), U128_HI(seed)));
...
template <typename T> struct city_hash_crc_t : public Hasher<city_hash_crc_t<T>, T> {
  public:
...
template <typename T> struct city_fingerprint_t : public Fingerprinter<city_fingerprint_t<T>, T> {
  public:
...
template <>
const city_hash_crc_128_t::hash_value_t city_hash_crc_128_t::operator()(void *buf, size_t len,
                                                                        city_hash_crc_128_t::seed_value_t seed) const {
    if (seed) {
        const uint128 &hash =
            CityHashCrc128WithSeed((const char *)buf, len, std::make_pair(U128_LO(seed), U128_HI(seed)));
