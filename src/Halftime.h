...
template <> halftime_seed_t as_seed_value(uint64_t hash) {
    halftime_seed_t seed;
    std::generate(seed.begin(), seed.end(), std::mt19937_64(hash));
    return seed;
}

template <> uint64_t as_hash_value(halftime_seed_t seed) { return seed[0]; }

#ifdef SUPPORT_INT128
template <> halftime_seed_t as_seed_value(uint128_t hash) {
...
template <> halftime_seed_t as_seed_value(uint256_t hash) {
...
template <> halftime_seed_t as_seed_value(uint512_t hash) {
    std::seed_seq seeds(hash.begin(), hash.end());
    std::mt19937_64 gen(seeds);
...
template <> uint128_t as_hash_value(halftime_seed_t seed) { return U128_NEW(seed[0], seed[1]); }

template <> uint256_t as_hash_value(halftime_seed_t seed) { return {seed[0], seed[1], seed[2], seed[3]}; }

template <> uint512_t as_hash_value(halftime_seed_t seed) {
    return {seed[0], seed[1], seed[2], seed[3], seed[4], seed[5], seed[6], seed[7]};
}

#endif

template <typename T> class halftime_hash_t : public Hasher<halftime_hash_t<T>, halftime_seed_t, uint64_t> {
  public:
...
template <>
const halftime_hash_64_t::hash_value_t halftime_hash_64_t::operator()(void *buf, size_t len,
                                                                      halftime_hash_64_t::seed_value_t seed) const {
...
template <>
const halftime_hash_128_t::hash_value_t halftime_hash_128_t::operator()(void *buf, size_t len,
                                                                        halftime_hash_128_t::seed_value_t seed) const {
    return halftime_hash::HalftimeHashStyle128(seed.data(), (const char *)buf, len);
}

template <>
const halftime_hash_256_t::hash_value_t halftime_hash_256_t::operator()(void *buf, size_t len,
                                                                        halftime_hash_256_t::seed_value_t seed) const {
    return halftime_hash::HalftimeHashStyle256(seed.data(), (const char *)buf, len);
}

template <>
const halftime_hash_512_t::hash_value_t halftime_hash_512_t::operator()(void *buf, size_t len,
                                                                        halftime_hash_512_t::seed_value_t seed) const {
