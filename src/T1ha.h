...
template <typename T, t1_hash_a A> class t1_hash_t : public Hasher<t1_hash_t<T, A>, uint64_t, T> {
  public:
...
template <>
const t1ha2_atonce_t::hash_value_t t1ha2_atonce_t::operator()(void *buf, size_t len,
                                                              t1ha2_atonce_t::seed_value_t seed) const {
    return t1ha2_atonce(buf, len, seed);
}

#if defined(SUPPORT_INT128)
template <>
const t1ha2_atonce128_t::hash_value_t t1ha2_atonce128_t::operator()(void *buf, size_t len,
                                                                    t1ha2_atonce128_t::seed_value_t seed) const {
    uint64_t hi = 0;
...
template <> const t1ha0_t::hash_value_t t1ha0_t::operator()(void *buf, size_t len, t1ha0_t::seed_value_t seed) const {
