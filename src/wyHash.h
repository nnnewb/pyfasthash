...
template <typename T> class wy_hash_t : public Hasher<wy_hash_t<T>, T> {
  public:
...
template <>
const wy_hash_32_t::hash_value_t wy_hash_32_t::operator()(void *buf, size_t len,
                                                          wy_hash_32_t::seed_value_t seed) const {
    return wyhash32(buf, len, seed);
}
template <>
const wy_hash_64_t::hash_value_t wy_hash_64_t::operator()(void *buf, size_t len,
                                                          wy_hash_64_t::seed_value_t seed) const {
