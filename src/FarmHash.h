...
template <typename T> class farm_hash_t : public Hasher<farm_hash_t<T>, T> {
  public:
...
template <>
const farm_hash_32_t::hash_value_t farm_hash_32_t::operator()(void *buf, size_t len,
                                                              farm_hash_32_t::seed_value_t seed) const {
    if (seed) {
...
template <>
const farm_hash_64_t::hash_value_t farm_hash_64_t::operator()(void *buf, size_t len,
                                                              farm_hash_64_t::seed_value_t seed) const {
    if (seed) {
...
template <>
const farm_hash_128_t::hash_value_t farm_hash_128_t::operator()(void *buf, size_t len,
                                                                farm_hash_128_t::seed_value_t seed) const {
    uint128_c_t hash;
...
template <typename T> class farm_fingerprint_t : public Fingerprinter<farm_fingerprint_t<T>, T> {
  public:
...
template <> const farm_fingerprint_32_t::fingerprint_t farm_fingerprint_32_t::operator()(void *buf, size_t len) const {
    return farmhash_fingerprint32((const char *)buf, len);
}

template <> const farm_fingerprint_64_t::fingerprint_t farm_fingerprint_64_t::operator()(void *buf, size_t len) const {
