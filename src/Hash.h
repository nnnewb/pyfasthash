...
namespace pybind11 {
    namespace detail {
        template <> struct type_caster<uint128_t> {
          public:
...
#if PY_MINOR_VERSION < 13
                _PyLong_AsByteArray((PyLongObject *)n.ptr(), (unsigned char *)&value, sizeof(uint128_t),
                                    IS_LITTLE_ENDIAN, PyLong_Unsigned);
#else
                PyLong_AsNativeBytes(n.ptr(), &value, sizeof(uint128_t), IS_LITTLE_ENDIAN);
#endif
                return !PyErr_Occurred();
            }

            static handle cast(uint128_t src, return_value_policy /* policy */, handle /* parent */) {
                return _PyLong_FromByteArray((const unsigned char *)&src, sizeof(uint128_t), IS_LITTLE_ENDIAN,
                                             PyLong_Unsigned);
            }
        };

        template <> struct type_caster<uint256_t> {
          public:
...
#if PY_MINOR_VERSION < 13
                _PyLong_AsByteArray((PyLongObject *)n.ptr(), (unsigned char *)&value, sizeof(uint256_t),
                                    IS_LITTLE_ENDIAN, PyLong_Unsigned);
#else
                PyLong_AsNativeBytes(n.ptr(), &value, sizeof(uint256_t), IS_LITTLE_ENDIAN);
#endif
            ...
            static handle cast(uint256_t src, return_value_policy /* policy */, handle /* parent */) {
                return _PyLong_FromByteArray((const unsigned char *)src.data(), sizeof(uint256_t), IS_LITTLE_ENDIAN,
                                             PyLong_Unsigned);
            }
        };

        template <> struct type_caster<uint512_t> {
          public:
...
#if PY_MINOR_VERSION < 13
                _PyLong_AsByteArray((PyLongObject *)n.ptr(), (unsigned char *)&value, sizeof(uint512_t),
                                    IS_LITTLE_ENDIAN, PyLong_Unsigned);
#else
                PyLong_AsNativeBytes(n.ptr(), &value, sizeof(uint512_t), IS_LITTLE_ENDIAN);
#endif
                return !PyErr_Occurred();
            }

            static handle cast(uint512_t src, return_value_policy /* policy */, handle /* parent */) {
                return _PyLong_FromByteArray((const unsigned char *)src.data(), sizeof(uint512_t), IS_LITTLE_ENDIAN,
                                             PyLong_Unsigned);
            }
        };
    } // namespace detail
...
template <typename T, typename S, typename H = S> class Hasher {
  public:
...
template <typename T, typename H> class Fingerprinter {
  public:
    ...
    static void Export(const py::module &m, const char *name) {
        py::class_<T>(m, name).def(py::init<>()).def("__call__", &T::CallWithArgs);
    }
};

void handle_data(PyObject *obj, std::function<void(const char *buf, Py_ssize_t len)> callback);

template <typename T, typename S = T> T as_hash_value(S from) { return static_cast<T>(from); }

template <typename S, typename T = S> S as_seed_value(T hash) { return hash; }

template <> uint128_t as_hash_value(uint64_t seed) { return uint128_t{seed}; }

template <> uint64_t as_seed_value(uint128_t hash) { return static_cast<uint64_t>(hash); }

template <> uint256_t as_hash_value(uint64_t seed) { return {seed}; }

template <> uint64_t as_seed_value(uint256_t hash) { return hash[0]; }

template <> uint64_t as_hash_value(uint256_t seed) { return seed[0]; }

template <> uint256_t as_seed_value(uint64_t hash) { return {hash, hash, hash, hash}; }

template <> uint256_t as_hash_value(uint128_t seed) {
    return {U128_LO(seed), U128_HI(seed), U128_LO(seed), U128_HI(seed)};
}

template <> uint128_t as_seed_value(uint256_t hash) { return U128_NEW(hash[0], hash[1]); }

template <> uint128_t as_hash_value(uint256_t seed) { return U128_NEW(seed[0], seed[1]); }

template <> uint256_t as_seed_value(uint128_t hash) {
...
template <typename T, typename H> py::object Fingerprinter<T, H>::CallWithArgs(py::args args, py::kwargs kwargs) {
    if (args.size() == 0) {
        throw std::invalid_argument("missed self argument");
    }
    ...
    std::for_each(std::next(args.begin()), args.end(), [&](const py::handle &arg) {
        handle_data(arg.ptr(),
                    [&](const char *buf, Py_ssize_t len) { results.push_back(fingerprinter((void *)buf, len)); });
    });
